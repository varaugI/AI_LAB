...
def validate_patterns():
    ...
    for letter, variants in LETTER_PATTERNS.items():
        if isinstance(variants[0], str):
            raise ValueError(f"{letter} must be a list of variant patterns, not a single pattern.")
        for variant in variants:
            if len(variant) != PATTERN_HEIGHT:
                raise ValueError(f"{letter} variant must have {PATTERN_HEIGHT} rows.")
            if any(len(row) != PATTERN_WIDTH for row in variant):
                raise ValueError(f"Every row for {letter} variant must have width {PATTERN_WIDTH}.")
