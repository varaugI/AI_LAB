...
def normalize_output(number):
    # Output can be from -MAX_VAL to MAX_VAL. Normalize to 0-1.
    return (number + MAX_VAL) / (2.0 * MAX_VAL)

def denormalize_output(number):
    return (number * 2.0 * MAX_VAL) - MAX_VAL

def create_subtraction_training_data(number_of_examples):
    inputs = []
    targets = []
    for _ in range(number_of_examples):
        first_number = random.uniform(0, MAX_VAL)
        second_number = random.uniform(0, MAX_VAL)
...
def main():
    random.seed(42)
    training_inputs, training_targets = create_subtraction_training_data(1_500)
    ...
    print("Training the neural network on SUBTRACTION...\n")
    network.train(training_inputs, training_targets, epochs=10_000, print_every=1000, batch_size=32)
    network.save("subtraction_network.json")
    print("\nTraining completed.")
    ...
    while True:
        ...
        second_text = input("Second number (0-100): ").strip()
